...
def generate_launch_description():
    ...
    log_config_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "carma_rosconsole.conf"
    )
    vehicle_config_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "VehicleConfigParams.yaml"
    )

    logging_env_var = SetEnvironmentVariable(
        "CARMA_ROS_LOGGING_CONFIG", generate_log_levels(log_config_file_path)
    )
    ...
    declare_vehicle_config_dir_arg = DeclareLaunchArgument(
        ...
        description="Path to vehicle configuration directory",
    )
    ...
    declare_load_type = DeclareLaunchArgument(
        name="load_type", default_value="noupdate"
    )

    single_pcd_path = LaunchConfiguration("single_pcd_path")
    declare_single_pcd_path = DeclareLaunchArgument(
        name="single_pcd_path", default_value="['/opt/carma/maps/pcd_map.pcd']"
    )
    ...
    declare_arealist_path = DeclareLaunchArgument(
        name="arealist_path", default_value="/opt/carma/maps/arealist.txt"
    )

    vector_map_file = LaunchConfiguration("vector_map_file")
    declare_vector_map_file = DeclareLaunchArgument(
        name="vector_map_file", default_value="/opt/carma/maps/vector_map.osm"
    )
    ...
    enable_guidance_plugin_validator = LaunchConfiguration(
    ...
    carma_src_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [get_package_share_directory("carma"), "/launch/carma_src.launch.py"]
        ),
        launch_arguments={
